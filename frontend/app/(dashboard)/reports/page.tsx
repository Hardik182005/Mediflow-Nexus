    ...
    <div className="space-y-6 max-w-[1400px] mx-auto animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[13px] text-black/40 mt-0.5">Export, schedule, and analyze custom data views.</p>
        </div>
                ...
                <div className="space-y-2.5">
                  {categories.map((cat) => (
                    <label key={cat} className="flex items-center gap-2.5 cursor-pointer group" onClick={() => toggleCategory(cat)}>
                      <div className={`w-4 h-4 rounded border-[1.5px] flex items-center justify-center transition-all ${selectedCategories.includes(cat) ? 'bg-black border-black' : 'border-black/20 group-hover:border-black/40'}`}>
                        {selectedCategories.includes(cat) && <CheckCircle2 size={10} className="text-white" />}
                      </div>
                      <span className="text-[13px] text-black/55 group-hover:text-black transition-colors">{cat}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <button
                    ...
                    className="w-full flex items-center justify-between bg-black/[0.02] border border-black/[0.08] rounded-lg px-3 py-2.5 text-[13px] text-black hover:border-black/20 transition-colors"
                  >
                    <span>{dateRange}</span>
                    <ChevronDown size={13} className={`text-black/30 transition-transform ${showDateDropdown ? "rotate-180" : ""}`} />
                  </button>
                  {showDateDropdown && (
                    <div className="absolute left-0 top-full mt-1 w-full bg-white border border-black/[0.1] rounded-xl py-1 z-20 shadow-xl">
                      {dateRanges.map((range) => (
                        <button
                          key={range}
                          onClick={() => { setDateRange(range); setShowDateDropdown(false); }}
                          className={`w-full text-left px-4 py-2.5 text-[12.5px] hover:bg-black/[0.03] transition-colors ${dateRange === range ? "text-black font-semibold" : "text-black/50"}`}
                        >
                          {range}
                        </button>
                      ))}
          ...
          <div className="bg-white border border-black/[0.07] rounded-2xl overflow-hidden">
            <div className="px-5 py-4 border-b border-black/[0.06] flex items-center justify-between">
              <h3 className="text-[14px] font-semibold text-black">
                Available Reports <span className="text-black/30 font-normal">({filteredReports.length})</span>
              </h3>
              <div className="flex items-center gap-2">
                <span className="text-[12px] text-black/40">Sort:</span>
                <button
                  onClick={() => setSortBy(sortBy === "newest" ? "oldest" : "newest")}
                  className="text-[12.5px] font-semibold text-black/60 flex items-center gap-1 hover:text-black transition-colors"
                >
            ...
            <div className="divide-y divide-black/[0.04]">
              {isLoading ? (
                <div className="p-12 flex flex-col items-center justify-center text-black/25 gap-3">
                  <Loader2 size={22} className="animate-spin" />
                  <p className="text-[13px]">Fetching intelligence data…</p>
                </div>
              ) : filteredReports.map((report, i) => (
                <div key={i} className="px-5 py-4 flex items-center justify-between hover:bg-black/[0.015] transition-colors group">
                  <div className="flex items-center gap-4">
                    <div className="w-9 h-9 rounded-xl bg-black/[0.04] border border-black/[0.07] flex items-center justify-center text-black/40">
                      {getCategoryIcon(report.category)}
                    </div>
                    <div>
                      <h4 className="text-[13.5px] font-semibold text-black">{report.name}</h4>
                      <div className="flex items-center gap-2 mt-0.5 text-[11.5px] text-black/35">
                        <span>{report.type}</span>
                        <span>·</span>
                        <span>{report.date}</span>
                        <span>·</span>
                        <span className="text-black/40">{report.category}</span>
                      </div>
                    </div>
                  </div>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setShowScheduleModal(false)} className="text-black/30 hover:text-black transition-colors"><X size={18} /></button>
              </div>
              <div className="space-y-3">
                <div>
                  <label className="text-[12px] text-black/50 font-semibold mb-1.5 block">Delivery Email *</label>
                ...
                <div>
                  <label className="text-[12px] text-black/50 font-semibold mb-1.5 block">Frequency</label>
                  <div className="grid grid-cols-3 gap-2">
                    {["Daily", "Weekly", "Monthly"].map((f) => (
                      <button key={f} onClick={() => setScheduleFrequency(f)} className={`flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-[12.5px] font-medium transition-all border ${scheduleFrequency === f ? "bg-black text-white border-black" : "bg-white border-black/10 text-black/50 hover:text-black hover:border-black/20"}`}>
                ...
                <div>
                  <label className="text-[12px] text-black/50 font-semibold mb-1.5 block">Format</label>
                  <div className="grid grid-cols-3 gap-2">
                    {["PDF", "CSV", "Excel"].map((fmt) => (
                      <button key={fmt} onClick={() => setScheduleFormat(fmt)} className={`py-2.5 rounded-lg text-[12.5px] font-medium transition-all border ${scheduleFormat === fmt ? "bg-black text-white border-black" : "bg-white border-black/10 text-black/50 hover:text-black hover:border-black/20"}`}>{fmt}</button>
                    ))}
                  </div>
                </div>
              </div>
              <div className="p-3 rounded-xl bg-black/[0.02] border border-black/[0.06] text-[11.5px] text-black/45 flex items-start gap-2">

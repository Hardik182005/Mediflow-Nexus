    ...
    <div className="space-y-6 max-w-[1600px] mx-auto animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[13px] text-black font-medium">Discover integrations, startups, and enterprise buyers.</p>
        </div>
        <div className="flex gap-3">
          <div className="relative w-64">
            ...
            <input
              ...
              className="w-full bg-white border border-black text-black text-[13px] rounded-lg pl-9 pr-4 py-2 focus:outline-none focus:border-black transition-all placeholder-black/20"
            />
          </div>
        </div>
      </div>
      ...
      {/* How It Works */}
      <div className="bg-white border border-black rounded-2xl p-6 flex flex-wrap items-center gap-x-8 gap-y-4 text-[12px] font-bold uppercase tracking-wider">
        ...
        <ArrowUpRight size={14} className="text-black hidden md:block" />
        <div className="flex items-center gap-3 text-black">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.map((item, i) => (
          <motion.div
            ...
            className="bg-white border border-black rounded-3xl p-6 shadow-sm hover:shadow-xl transition-all group"
          >
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold text-black uppercase tracking-widest">{item.category}</span>
              </div>
            </div>
            
            <h3 className="text-[18px] font-bold text-black tracking-tight font-serif">{item.name}</h3>
            <p className="text-[13px] text-black mt-2 leading-relaxed line-clamp-2 font-medium">{item.description}</p>

            {/* Tags */}
            <div className="flex flex-wrap gap-1.5 mt-4">
              {item.tags.map((tag) => (
                <span key={tag} className="text-[10px] px-2 py-1 rounded-md bg-white border border-black text-black font-bold uppercase">{tag}</span>
              ))}
            </div>
            
            <div className="mt-8 flex items-center justify-between border-t border-black pt-5">
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-1 text-[13px] font-bold text-black">
                  <Star size={14} className="text-black fill-black" />
                  {item.rating}
                </div>
                <div className="text-[12px] text-black font-bold flex items-center gap-1 uppercase tracking-tighter">
        ...
        {introModal && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-md bg-white border border-black rounded-3xl p-8 shadow-2xl"
            >
              <div className="flex items-center justify-between mb-8">
                <h3 className="text-xl font-bold text-black font-serif">Request Intro</h3>
                <button onClick={() => setIntroModal(null)} className="text-black hover:text-black transition-colors">
                  <X size={20} />
                </button>
              </div>

              <div className="bg-white border border-black p-5 rounded-2xl mb-8 flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[12px] text-black font-bold uppercase tracking-wider">{introModal.category}</p>
                </div>
              </div>

              <div className="space-y-6">
                <div>
                  <label className="text-[12px] font-bold text-black uppercase tracking-widest mb-2 block">Your Enterprise Email</label>
                  <input
                    type="email"
                    value={introEmail}
                    onChange={(e) => setIntroEmail(e.target.value)}
                    className="w-full bg-white border border-black rounded-xl px-4 py-3 text-[14px] text-black focus:outline-none focus:ring-2 focus:ring-black/5"
                    placeholder="name@organization.com"
                  />
                </div>
                <div>
                  <label className="text-[12px] font-bold text-black uppercase tracking-widest mb-2 block">Message</label>
                  <textarea
                    value={introMessage}
                    onChange={(e) => setIntroMessage(e.target.value)}
                    className="w-full bg-white border border-black rounded-xl px-4 py-3 text-[14px] text-black focus:outline-none focus:ring-2 focus:ring-black/5 min-h-[100px] resize-none"
                    placeholder="Tell them about your interest..."
                    rows={3}
                  />
                </div>

                <div className="flex items-start gap-3 text-[11px] text-black font-medium leading-relaxed bg-white p-4 rounded-xl">
                  <Mail size={16} className="mt-0.5 flex-shrink-0 text-black" />

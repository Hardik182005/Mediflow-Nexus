    ...
    <div className="space-y-6 max-w-[1600px] mx-auto animate-fade-in font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[13px] text-black font-medium">Manage intake, verifications, and staff performance.</p>
        </div>
        <div className="flex gap-3">
          <button onClick={handleExportData} className="bg-white border border-black text-black px-4 py-2 rounded-lg text-sm font-bold hover:bg-black/5 transition-all flex items-center gap-2 shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-black rounded-3xl p-6 shadow-sm lg:col-span-2">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-[11px] text-black uppercase tracking-widest font-bold mt-1">Today's Volume</p>
            </div>
            <div className="relative">
              <button onClick={() => setShowChartMenu(!showChartMenu)} className="p-2 rounded-lg hover:bg-black/5 text-black transition-colors">
                <MoreHorizontal size={18} />
              </button>
              {showChartMenu && (
                <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-black rounded-2xl py-2 z-30 shadow-2xl overflow-hidden">
                  <button onClick={() => handleChartAction("fullReport")} className="w-full text-left px-4 py-2.5 text-xs font-bold text-black hover:bg-black/5 hover:text-black flex items-center gap-2">
                    <BarChart3 size={14} /> View Full Report
                  </button>
                  <button onClick={() => handleChartAction("refresh")} className="w-full text-left px-4 py-2.5 text-xs font-bold text-black hover:bg-black/5 hover:text-black flex items-center gap-2">
                    <RefreshCw size={14} /> Refresh Data
                  </button>
                  <button onClick={() => handleChartAction("export")} className="w-full text-left px-4 py-2.5 text-xs font-bold text-black hover:bg-black/5 hover:text-black flex items-center gap-2 border-t border-black">
        ...
        {/* Auth Queue */}
        <div className="bg-white border border-black rounded-3xl overflow-hidden shadow-sm flex flex-col">
          <div className="p-6 border-b border-black flex items-center justify-between">
            <div>
              <h3 className="text-[15px] font-bold text-black">Action Required</h3>
              <p className="text-[11px] text-black font-bold uppercase tracking-widest mt-1">Pending Auths</p>
            </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-black">
                  <th className="px-6 py-4 text-[10px] font-bold text-black uppercase tracking-widest">Patient</th>
                ...
                {[
                  ...
                  <tr key={i} className="hover:bg-white transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`text-[10px] font-bold px-2 py-1 rounded uppercase ${item.urgent ? 'bg-black text-white' : 'bg-black/5 text-black'}`}>
                        {item.status}
                      </span>
                    </td>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setShowWorkflowModal(false)} className="text-black hover:text-black"><X size={20} /></button>
              </div>
              <div className="space-y-4">
                <div>
                  <label className="text-[12px] font-bold text-black uppercase tracking-widest mb-2 block">Workflow Name</label>
                  <input value={workflowName} onChange={(e) => setWorkflowName(e.target.value)} className="w-full bg-white border border-black rounded-xl px-4 py-3 text-[14px] text-black focus:outline-none focus:ring-2 focus:ring-black/5" placeholder="e.g. Insurance Pre-Check" />
                </div>
                <div>
                  <label className="text-[12px] font-bold text-black uppercase tracking-widest mb-2 block">Category</label>
                  <select value={workflowType} onChange={(e) => setWorkflowType(e.target.value)} className="w-full bg-white border border-black rounded-xl px-4 py-3 text-[14px] text-black focus:outline-none focus:ring-2 focus:ring-black/5 appearance-none">
                ...
                <div>
                  <label className="text-[12px] font-bold text-black uppercase tracking-widest mb-2 block">Priority Level</label>

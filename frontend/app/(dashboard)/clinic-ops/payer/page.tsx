        ...
        {payers.map((p, i) => (
          <motion.div key={p.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="bg-white border border-black rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              <div><h3 className="text-sm font-semibold text-white">{p.payerName}</h3><p className="text-xs text-black">{p.totalClaims} total claims</p></div>
              <div className="w-12 h-12 rounded-xl flex items-center justify-center text-sm font-bold bg-white border border-black text-white">{p.performanceScore}</div>
            </div>

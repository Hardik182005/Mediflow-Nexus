...
gcloud run deploy "${SERVICE_NAME}" \
  ...
  --max-instances 10 \
  --timeout 300 \
  --set-env-vars "NODE_ENV=production,NEXT_PUBLIC_SUPABASE_URL=${NEXT_PUBLIC_SUPABASE_URL},NEXT_PUBLIC_SUPABASE_ANON_KEY=${NEXT_PUBLIC_SUPABASE_ANON_KEY},SUPABASE_SERVICE_ROLE_KEY=${SERVICE_KEY}${OPENAI_KEY:+,OPENAI_API_KEY=${OPENAI_KEY}}${GROQ_KEY:+,GROQ_API_KEY=${GROQ_KEY}}${ELEVENLABS_KEY:+,ELEVENLABS_API_KEY=${ELEVENLABS_KEY}}" \
  --quiet

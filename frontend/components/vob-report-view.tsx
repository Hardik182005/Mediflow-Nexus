    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center gap-4">
          <div className={`p-3 rounded-xl ${risk.banner} text-white`}>
            <risk.icon size={24} />
          </div>
          <div>
        ...
        <div className="text-right">
          <p className="text-[10px] text-black/40 uppercase font-bold tracking-widest">Est. Reimbursement</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-black/[0.07] rounded-2xl p-6">
            <div className="flex items-center gap-2 mb-6 border-b border-black/[0.07] pb-4">
              <User size={18} className="text-black/40" />
              <h3 className="text-sm font-bold text-black uppercase tracking-wider">Patient Information</h3>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {[
                ...
                <div key={i}>
                  <p className="text-[10px] text-black/40 uppercase font-bold mb-1">{item.label}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white border border-black/[0.07] rounded-2xl p-6">
              <div className="flex items-center gap-2 mb-4 border-b border-black/[0.07] pb-4">
                <ShieldCheck size={18} className="text-black" />
                <h3 className="text-sm font-bold text-black uppercase tracking-wider">Coverage Summary</h3>
              </div>
              <div className="space-y-4">
                <div className="flex justify-between items-end">
                  <div>
                    <p className="text-[10px] text-black/40 uppercase font-bold">Sum Insured</p>
                    <p className="text-lg font-bold text-black">{report.coverage_summary.sum_insured}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] text-black/40 uppercase font-bold">Remaining</p>
                    <p className="text-lg font-bold text-black">{report.coverage_summary.remaining_balance}</p>
                  </div>
                </div>
            ...
            <div className="bg-white border border-black/[0.07] rounded-2xl p-6">
              <div className="flex items-center gap-2 mb-4 border-b border-black/[0.07] pb-4">
                ...
                <h3 className="text-sm font-bold text-black uppercase tracking-wider">Hospitalization</h3>
              </div>
              <div className="space-y-3">
                {[
                  ...
                  <div key={i} className="flex justify-between items-center text-xs py-2 border-b border-black/[0.05]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white border border-black/[0.07] rounded-2xl p-6">
              <div className="flex items-center gap-2 mb-4 border-b border-black/[0.07] pb-4">
                ...
                <h3 className="text-sm font-bold text-black uppercase tracking-wider">Financials</h3>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 rounded-xl bg-black/[0.02] border border-black/[0.07]">
                  <p className="text-[9px] text-black/40 uppercase font-bold mb-1">Co-payment</p>
                  <p className="text-sm font-bold text-black">{report.financials.copayment_percent}</p>
                </div>
                <div className="p-3 rounded-xl bg-black/[0.02] border border-black/[0.07]">
                  <p className="text-[9px] text-black/40 uppercase font-bold mb-1">Deductible</p>
                  <p className="text-sm font-bold text-black">{report.financials.deductible}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-black/[0.07] rounded-2xl p-6">
              <div className="flex items-center gap-2 mb-4 border-b border-black/[0.07] pb-4">
                <Ban size={18} className="text-black/40" />
                <h3 className="text-sm font-bold text-black uppercase tracking-wider">Major Exclusions</h3>
              </div>
              <div className="flex flex-wrap gap-2">
                {report.exclusions.map((e, i) => (
                  <span key={i} className="text-[10px] px-2 py-1 rounded bg-black/[0.03] text-black/70 border border-black/10">
                    {e}
                  </span>
                ))}
              </div>
        ...
        <div className="space-y-6">
          <div className={`p-6 rounded-2xl border ${risk.bg} ${risk.border}`}>
            <div className="flex items-center gap-2 mb-6">
              ...
              <h3 className="text-sm font-bold text-black uppercase tracking-wider">AI Recommendations</h3>
            </div>
            <div className="space-y-4">
              {report.ai_risk_assessment.recommendations.map((r, i) => (
                <div key={i} className="flex items-start gap-3">
                  <div className={`w-1 h-1 rounded-full mt-2 shrink-0 ${risk.banner}`} />
                  <p className="text-xs text-black/70 leading-relaxed font-medium">{r}</p>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white border border-black/[0.07] rounded-2xl p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-bold text-black uppercase tracking-wider">Network Status</h3>
            </div>
            <div className="p-4 rounded-xl bg-black/[0.03] border border-black/10 text-center">
              <p className="text-lg font-bold text-black">{report.network_status.status}</p>
          ...
          <div className="flex gap-3">
            <button className="flex-1 p-3 rounded-xl bg-black/[0.02] border border-black/10 text-black/40 hover:text-black transition-all flex items-center justify-center gap-2">

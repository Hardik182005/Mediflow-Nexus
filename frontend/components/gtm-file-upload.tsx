    ...
    <div className="space-y-5">
      ...
      >
        <input
          ...
          className="hidden"
        />
        <div className={`w-14 h-14 rounded-2xl border flex items-center justify-center transition-all ${dragging ? "bg-black/10 border-black/30" : "bg-black/[0.03] border-black/[0.08]"}`}>
        ...
        {files.length < MAX_FILES && (
          <div className="flex items-center gap-1.5 text-[11px] text-black/30 font-bold uppercase tracking-widest">
      ...
      {errors.length > 0 && (
        <div className="space-y-1">
          {errors.map((e, i) => (
            <p key={i} className="text-xs text-black/60 flex items-center gap-2 px-3 py-2 rounded-lg bg-black/[0.02] border border-black/[0.08] font-medium">
              ⚠ {e}
            </p>
          ))}
        </div>
        ...
        <div className="space-y-2">
          ...
          {files.map((file) => (
            <div key={file.id} className="bg-white border border-black/[0.07] rounded-2xl p-3 flex items-center gap-3">
              <div className="w-9 h-9 rounded-lg bg-black/[0.04] border border-black/[0.08] flex items-center justify-center text-black/50">
                <FileIcon ext={file.ext} size={16} />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm text-black font-bold truncate">{file.name}</p>
                <p className="text-[11px] text-black/30 font-medium">{formatSize(file.sizeKB)}</p>
              </div>
              {/* Editable label */}
              <select
                value={file.label}
                onChange={(e) => updateLabel(file.id, e.target.value)}
                className="bg-black/[0.03] border border-black/[0.08] text-black/60 text-[11px] rounded-lg px-2 py-1.5 outline-none focus:border-black/20 cursor-pointer font-bold"
              >
      ...
      <div className="space-y-2">
        ...
        <textarea
          ...
          rows={3}
          className="w-full bg-black/[0.02] border border-black/[0.08] rounded-xl p-3 text-sm text-black/80 placeholder:text-black/20 outline-none focus:border-black/20 resize-none transition-colors"
        />
      </div>
      ...
      <button
        ...
        className="w-full btn-primary flex items-center justify-center gap-2 py-3.5 text-sm font-bold disabled:opacity-30 disabled:cursor-not-allowed"
      >
        {isLoading ? (
          <>
